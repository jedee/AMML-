...
import { PayrollView } from '../components/amml/PayrollView';
import { ArrowLeft, Wallet } from 'lucide-react';

export const Route = createRoute({
  getParentRoute: () => AmmlLayoutRoute,
  path: '/payroll',
  component: PayrollRouteComponent,
});

function PayrollRouteComponent() {
  return (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 font-mono select-none">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-amml-green uppercase tracking-widest">Compilers wages</span>
          </div>
          <h1 className="text-lg font-bold text-white tracking-wider mt-1.5 uppercase">Workforce Payroll ledger</h1>
        </div>
        
        <div className="flex items-center gap-2 text-xs text-amml-muted bg-amml-panel border border-amml-line px-3 py-1.5 rounded">
          <Wallet className="h-4 w-4 text-amml-green" />
          <span>WAGES LEDGER: ESTIMATED</span>
        </div>
      </div>
